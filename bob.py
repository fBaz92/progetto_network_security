...
def main():
    ...
    print("In attesa della connessione di Alice...")
    conn, addr = server.accept()
    print(f"Alice connessa da {addr}")
    
    try:
        # Ricevi la chiave pubblica di Alice
        alice_public_key = int(conn.recv(1024).decode())
        print(f"Ricevuta chiave pubblica DH da Alice: {alice_public_key}")
